    ...
    <header className="fixed top-0 left-0 right-0 z-40 bg-[#0A0A0F]/90 backdrop-blur-xl border-b border-white/5 py-3">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center relative">
        ...
        <nav className="hidden lg:flex items-center space-x-2 bg-white/[0.03] p-1.5 rounded-full border border-white/10">
          ...
          {/* Interactive Dashboards Dropdown */}
          <div className="relative">
            <button
              onClick={() => setIsDashboardDropdownOpen(!isDashboardDropdownOpen)}
              className="px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-300 cursor-pointer flex items-center space-x-1.5"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isDashboardDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-48 bg-[#0E0E12] border border-white/10 p-2 shadow-2xl rounded-xl z-50">
                {dashboardOptions.map(opt => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-300 rounded-lg cursor-pointer"
                  >
                    {opt.name}
                  </button>
                ))}
